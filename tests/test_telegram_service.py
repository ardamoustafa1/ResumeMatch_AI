...
async def test_send_message_chat_not_found(mocker):
    mock_post = mocker.patch("httpx.AsyncClient.post")
    mock_response = mocker.Mock(spec=Response)
    mock_response.json.return_value = {"ok": False, "error_code": 400, "description": "chat not found"}
    mock_post.return_value = mock_response
...
async def test_send_message_other_error(mocker):
    mock_post = mocker.patch("httpx.AsyncClient.post")
    mock_response = mocker.Mock(spec=Response)
    mock_response.json.return_value = {"ok": False, "error_code": 500, "description": "Server Error"}
    mock_post.return_value = mock_response
...
async def test_send_message_timeout(mocker):
    mock_post = mocker.patch("httpx.AsyncClient.post", side_effect=TimeoutException("timeout"))
...
async def test_send_message_request_error(mocker):
    mock_post = mocker.patch("httpx.AsyncClient.post", side_effect=RequestError("error", request=mocker.Mock(spec=Request)))
...
async def test_verify_telegram_config(mocker):
    mocker.patch("backend.services.telegram_service._send_message", return_value={"ok": True})
    assert await verify_telegram_config("bot", "chat") is True


async def test_verify_telegram_config_fail(mocker):
    mocker.patch("backend.services.telegram_service._send_message", side_effect=TelegramError("fail"))
...
async def test_send_error_notification_fail(mocker):
    mock_send = mocker.patch("backend.services.telegram_service._send_message", side_effect=Exception("fail"))
    # Should not raise
    await send_error_notification("chat", "bot", "id123", "Some error")
...
async def test_send_analysis_complete(mocker):
    ...
    result = FullAnalysisResult(
        match_result=MatchResult(score=85, match_reasoning="Good", matched_skills=["Python", "Go"], missing_skills=["Rust"], improvement_suggestions=[]),
        outreach_messages=OutreachMessages(dm_first_contact="Hi", dm_follow_up="Bump", connection_note="Connect")
    )
