...
async def test_extract_text_from_image_success(mocker):
    mock_image_open = mocker.patch(
        "backend.services.extraction_service.image_module.open"
    )
    mock_img = MagicMock()
    mock_img.width = 100
    mock_img.height = 100
    ...
    mock_tesseract = mocker.patch(
        "backend.services.extraction_service.pytesseract_module.image_to_string"
    )
...
async def test_extract_text_from_image_limit_exceeded(mocker):
    ...
    with pytest.raises(
        ValueError, match="Could not parse Image file: Image dimensions exceed"
    ):
...
async def test_extract_document_text_routing(mocker):
    mocker.patch(
        "backend.services.extraction_service.extract_text_from_pdf",
        return_value="PDF",
    )
    mocker.patch(
        "backend.services.extraction_service.extract_text_from_image",
        return_value="IMG",
    )
