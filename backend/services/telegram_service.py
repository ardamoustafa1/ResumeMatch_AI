...
async def send_error_notification(
    chat_id: str, bot_token: str, analysis_id: str, error: str
) -> None:
    """
...
async def send_analysis_complete(
    ...
    safe_recruiter = (
        format_message_for_telegram(recruiter_name)
        if recruiter_name
        else "Not specified"
    )
    ...
    if result.match_result:
        matched = format_message_for_telegram(
            ", ".join(result.match_result.matched_skills[:5]), truncate_len=100
        )
        missing = format_message_for_telegram(
            ", ".join(result.match_result.missing_skills[:5]), truncate_len=100
        )
    else:
        matched = "None"
        missing = "None"
